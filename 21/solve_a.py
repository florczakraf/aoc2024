...
def get_path_len(g, steps, epoch, start):
    if not steps:
        return 0
    ...
    if epoch == 0:
        ...
        for candidate in set(permutations(keys)):
            ...
            v = get_path_len(g=arrows, steps=candidate + ("A", ), epoch=epoch-1, start=arrows["A"])
            if v < presses:
                presses = v

    return presses + get_path_len(g, steps[1:], epoch, neighbor)


s = 0
for l in lines:
    path_len = get_path_len(g=numpad, steps=l, epoch=2, start=numpad["A"])
    num = int(l[:-1])
    s += path_len * num
